 ...
 *
 */
Status interp(NODE *n)
{
   Status s;         /* returned error value      */
   ...
   switch(n -> kind){
      ...
      case N_CREATETABLE:            /* for CreateTable() */
         {
            ...
            s = pSmm->CreateTable(n->u.CREATETABLE.relname, nattrs, 
                  attrInfos);
            break;
         }   
      ...
      case N_CREATEINDEX:            /* for CreateIndex() */
         ...
         s = pSmm->CreateIndex(n->u.CREATEINDEX.relname,
               n->u.CREATEINDEX.attrname);
         break;

      case N_DROPINDEX:            /* for DropIndex() */

         s = pSmm->DropIndex(n->u.DROPINDEX.relname,
               n->u.DROPINDEX.attrname);
         break;

      case N_DROPTABLE:            /* for DropTable() */

         s = pSmm->DropTable(n->u.DROPTABLE.relname);
         break;
      ...
      case N_QUERY:            /* for Query() */
         {
            ...
            s = pQlm->Select(nSelAttrs, relAttrs,
      ...
      case N_INSERT:            /* for Insert() */
         {
            ...
            s = pQlm->Insert(n->u.INSERT.relname,
                  nValues, values);
            break;
         }   
      ...
      case N_DELETE:            /* for Delete() */
         {
            ...
            s = pQlm->Delete(n->u.DELETE.relname,
                  nConditions, conditions);
            break;
         }   
      ...
      case N_UPDATE:            /* for Update() */
         {
            ...
            s = pQlm->Update(n->u.UPDATE.relname, relAttr, bIsValue, 
...
static void mk_rel_attr(NODE *node, RelAttr &relAttr)
{
   relAttr.rel_name = node->u.RELATTR.relname;
   relAttr.attr_name = node->u.RELATTR.attrname;
}
...
static int mk_conditions(NODE *list, int max, Condition conditions[])
{
   ...
   for(i = 0; list != NULL; ++i, list = list -> u.LIST.next){
      ...
      current = list -> u.LIST.curr;
      conditions[i].l_attr.rel_name =
         current->u.CONDITION.lhsRelattr->u.RELATTR.relname;
      conditions[i].l_attr.attr_name =
      ...
      if (current->u.CONDITION.rhsRelattr) {
         ...
         conditions[i].r_is_attr = false;
         mk_value(current->u.CONDITION.rhsValue, conditions[i].r_value);
      }
   }
